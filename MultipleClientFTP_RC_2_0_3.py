...
def log_record(error_msg):
    ...
    print(f"log data collect : {log_data_collect[1]}")
    ...
    with open("datalog.log", "w") as file:
        ...
        if lower_log_size.upper() == "YES":
            if len_of_log_data_collect >= 2:
                count = 0
                for items in log_data_collect:
                    if log_data_collect[count][19:] == msg_will_write:
                        if log_data_collect[len_of_log_data_collect-1][19:] == msg_will_write:
                            print("masuk 1")
